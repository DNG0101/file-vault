...
function clearTimers() {
  ['role', 'approval', 'adminPoll', 'userPoll'].forEach(k => {
    if (ST.timers[k]) clearInterval(ST.timers[k]);
  });
}
    ...
    .then(users => {
      ...
      let html = active.map(u => `
        <div class="approval-item">
          ...
          <div style="display:flex;gap:6px;">
            ${u.status === 'pending' ? `
              <select class="role-${u.email}">
                <option value="full">Full</option><option value="delete">Delete</option>
                <option value="upload">Upload Only</option><option value="download">Download</option>
            ...
            ${u.status === 'approved' ? `
              <select class="role-${u.email}">
                ...
                <option value="upload" ${u.role === 'upload' ? 'selected' : ''}>Upload Only</option>
...
function loadLogs() {
  fetch(`${WORKER_BASE}/admin/logs?limit=100`, { headers: getAuthHeaders() })
    .then(r => r.json())
    .then(d => {
      D.logsCt.innerHTML = d.logs.length ? d.logs.map(l => `<div style="padding:6px 0;border-bottom:1px solid var(--border);font-size:0.8rem;"><strong>${new Date(l.ts).toLocaleString()}</strong> ${l.actor} <span style="color:var(--accent)">${l.action}</span> ${l.target || ''}</div>`).join('') : '<p>No logs yet.</p>';
    });
}
...
function loadShares() {
  D.shareCt.innerHTML = '<p>Create a share link from a file card (🔗 Share).</p>';
}
...
function toggleSelect(id, checked) {
  if (checked) ST.sel.add(id);
  else ST.sel.delete(id);
  updateBulkBar();
  document.querySelector(`.card[data-public-id="${id}"]`)?.classList.toggle('selected', checked);
}
...
async function uploadFile(file, existPid = null) {
  ...
  const sig = ST.uploadCtrl.signal;
    ...
    while (up < file.size) {
      ...
      while (retries-- && !done) {
        const cr = await fetch(`${WORKER_BASE}/upload-chunk/${pid}`, {
          method: 'PUT',
          headers: { 'Content-Range': `bytes ${up}-${end}/${file.size}`, ...getAuthHeaders() },
          body: chunk,
          signal: sig
        });
      ...
      if (!done) {
        const sr = await fetch(`${WORKER_BASE}/upload-status/${pid}`, { headers: getAuthHeaders(), signal: sig });
...
function updateProg(u, t) {
  const p = Math.round(u / t * 100);
  D.progFill.style.width = p + '%';
  D.pctDisp.textContent = p + '%';
  D.sizeDisp.textContent = `${fmtSz(u)} / ${fmtSz(t)}`;
}
const sleep = ms => new Promise(r => setTimeout(r, ms));
D.cancelBtn.addEventListener('click', () => {
  if (ST.uploadCtrl) { ST.uploadCtrl.abort(); ST.uploadCtrl = null; D.progBox.classList.add('hidden'); }
});
...
D.btnFull.addEventListener('click', () => {
  if (document.fullscreenElement) document.exitFullscreen();
  else D.prevOv.requestFullscreen();
});
...
async function deleteFile(pid) {
  ...
  const r = await fetch(`${WORKER_BASE}/delete`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
    body: JSON.stringify({ publicId: pid })
  });
  if (r.ok) {
    ST.files = ST.files.filter(f => f.publicId !== pid);
    ST.sel.delete(pid);
    renderFiles();
    updateBulkBar();
...
function replaceFile(pid) {
  ...
  inp.onchange = async () => {
    ...
    if (!file) return;
    const ir = await fetch(`${WORKER_BASE}/update`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
      body: JSON.stringify({ publicId: pid, fileName: file.name, fileSize: file.size, fileType: file.type || 'application/octet-stream' })
    });
...
async function renameFile(pid) {
  ...
  const r = await fetch(`${WORKER_BASE}/rename`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
    body: JSON.stringify({ publicId: pid, newName })
  });
...
async function shareFile(pid) {
  ...
  const r = await fetch(`${WORKER_BASE}/share/create`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
    body: JSON.stringify({ publicId: pid, expiresIn: (parseInt(expires || '1', 10) * 3600), maxDownloads, label })
  });
  const d = await r.json();
  if (r.ok) {
    toast('Share link created!');
    navigator.clipboard.writeText(d.shareUrl).then(() => toast('Link copied!'));
...
function startRolePoll() {
  ...
  ST.timers.role = setInterval(async () => {
    ...
    if (i.role !== ST.role) {
      ST.role = i.role;
      applyRoleUI();
      toast(`Permissions changed to ${ST.role}`);
    }
  }, 8000);
}
function startApprovalPoll() {
  ...
  ST.timers.approval = setInterval(async () => {
    ...
    if (i.approved) {
      ST.approved = true;
      ST.role = i.role;
      clearInterval(ST.timers.approval);
...
function startAdminPoll() {
  ...
  const poll = async () => {
    ...
    const r = await fetch(`${WORKER_BASE}/admin/poll?since=${lastTs}&timeout=15`, { headers: getAuthHeaders() });
    const d = await r.json();
    if (d.changed) {
      lastTs = d.ts;
      loadPendingCount();
...
function startUserPoll() {
  ...
  const poll = async () => {
    if (!ST.token || !ST.approved) return;
    const r = await fetch(`${WORKER_BASE}/poll?utoken=${ST.token}&since=${lastTs}&timeout=15`, { headers: getAuthHeaders() });
    const d = await r.json();
    if (d.changed) {
      lastTs = d.ts;
      fetchFiles();
...
['dragenter', 'dragover'].forEach(eventName => {
  D.dropzone.addEventListener(eventName, () => D.dropzone.classList.add('dragover'), false);
});
['dragleave', 'drop'].forEach(eventName => {
  D.dropzone.addEventListener(eventName, () => D.dropzone.classList.remove('dragover'), false);
});
