...
(function(){
  ...
  D.btnDark.addEventListener('click', () => {
    ...
    if (ST.token && ST.isAdmin) {
      fetch(`${WORKER_BASE}/admin/set-engine`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...getAuthHeaders() }, body: JSON.stringify({ engine: 'd1' }) }).catch(e=>console.warn);
    }
  });
})();
...
async function handleSharePreview() {
  ...
  if (!r.ok) { toast('Invalid or expired share link', 'error'); showLogin(); return; }
  const data = await r.json();
  D.grid.innerHTML = `<div style="grid-column:1/-1;text-align:center;padding:60px;"><div class="file-icon" style="font-size:4rem">${icn(data.mimeType)}</div><h2>${esc(data.fileName)}</h2><p>${fmtSz(data.fileSize)}</p><button class="btn btn-primary" id="shareDownloadBtn">⬇ Download</button><button class="btn btn-outline" onclick="location.href='/'">🔐 Login to Vault</button></div>`;
  document.getElementById('shareDownloadBtn')?.addEventListener('click', () => { window.location.href = `${WORKER_BASE}/share/download?token=${ST.shareToken}`; });
  D.dropzone.style.display = 'none'; document.querySelector('.hint-text').style.display = 'none'; D.adminBar.classList.add('hidden'); D.userUI.innerHTML = '';
}
...
function showPending() {
  D.adminBar.classList.add('hidden'); D.userUI.innerHTML = '';
  D.dropzone.style.display = 'none'; document.querySelector('.hint-text').style.display = 'none';
...
function render() {
  ...
  showMain(); applyRoleUI(); fetchFiles(); startRolePoll(); startUserPoll();
}
...
async function syncFiles() { toast('Syncing…','info'); const r = await fetch(`${WORKER_BASE}/sync`,{method:'POST',headers:getAuthHeaders()}); if(r.ok) { await fetchFiles(); toast('Sync done','success'); } else toast('Sync failed','error'); }
async function adminLogout() { if(!confirm('Logout as admin? Vault will continue to work.')) return; localStorage.removeItem('vtoken'); ST.token = null; clearTimers(); showLogin(); toast('Admin logged out'); }
...
function loadPending() {
  fetch(`${WORKER_BASE}/admin/pending`,{headers:getAuthHeaders()}).then(r=>r.json()).then(users => {
    D.listAppr.innerHTML = users.length ? users.map(u => `<div class="approval-item"><span>${u.email}</span><div style="display:flex;gap:6px;"><select class="role-sel"><option value="full">Full</option><option value="delete">Delete</option><option value="upload">Upload Only</option><option value="download">Download Only</option><option value="read">Read Only</option><option value="none">None</option></select><button class="btn btn-sm" style="background:var(--success);color:#fff;" data-action="approve" data-email="${u.email}">✅ Approve</button><button class="btn btn-sm" style="background:var(--danger);color:#fff;" data-action="deny" data-email="${u.email}">❌ Deny</button></div></div>`).join('') : '<p>No pending users.</p>';
...
function loadAllUsers() {
  fetch(`${WORKER_BASE}/admin/users/all`,{headers:getAuthHeaders()}).then(r=>r.json()).then(users => {
    ...
    let html = active.map(u => `<div class="approval-item"><span>${u.email} <span class="status-badge status-${u.status}">${u.status}</span> ${u.role?`(${u.role})`:''}</span><div style="display:flex;gap:6px;">${u.status==='pending'?`<select class="role-${u.email}"><option value="full">Full</option><option value="delete">Delete</option><option value="upload">Upload Only</option><option value="download">Download</option><option value="read">Read</option><option value="none">None</option></select><button class="btn btn-sm btn-success" data-action="approve" data-email="${u.email}">✅ Approve</button>`:''}${u.status==='approved'?`<select class="role-${u.email}"><option value="full" ${u.role==='full'?'selected':''}>Full</option><option value="delete" ${u.role==='delete'?'selected':''}>Delete</option><option value="upload" ${u.role==='upload'?'selected':''}>Upload Only</option><option value="download" ${u.role==='download'?'selected':''}>Download</option><option value="read" ${u.role==='read'?'selected':''}>Read</option><option value="none" ${u.role==='none'?'selected':''}>None</option></select><button class="btn btn-sm btn-primary" data-action="update" data-email="${u.email}">Update</button><button class="btn btn-sm btn-danger" data-action="revoke" data-email="${u.email}">Revoke</button>`:''}</div></div>`).join('');
    if(revoked.length>0) { html += '<hr><h4 style="margin-top:12px;margin-bottom:8px;">Revoked Users</h4>'; html += revoked.map(u => `<div class="approval-item"><span>${u.email} <span class="status-badge status-revoked">revoked</span> ${u.role?`(${u.role})`:''}</span><div style="display:flex;gap:6px;"><select class="role-${u.email}"><option value="full">Full</option><option value="delete">Delete</option><option value="upload">Upload Only</option><option value="download">Download</option><option value="read">Read</option><option value="none">None</option></select><button class="btn btn-sm btn-warn" data-action="reapprove" data-email="${u.email}">Re‑approve</button><button class="btn btn-sm btn-danger" disabled>Revoke</button></div></div>`).join(''); }
...
function renderFiles() {
  ...
  D.grid.innerHTML = '';
  if(Object.keys(grouped).length === 0) { D.grid.innerHTML = '<p style="text-align:center;padding:30px;">No files yet.</p>'; return; }
...
// Drag & Drop
['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => { D.dropzone.addEventListener(eventName, preventDefaults, false); });
...
document.addEventListener('paste', (e) => { const items = e.clipboardData.items; for (let i = 0; i < items.length; i++) { if (items[i].kind === 'file') { const file = items[i].getAsFile(); if (file) uploadFile(file); break; } } });
document.addEventListener('keydown', (e) => { if (e.ctrlKey && e.key === 'u') { e.preventDefault(); D.fileInp.click(); } if (e.ctrlKey && e.key === 'f') { e.preventDefault(); D.search.focus(); } if (e.key === 'Escape') closePreview(); });
init();
